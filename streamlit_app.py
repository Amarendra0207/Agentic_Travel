"""Streamlit web application for AI Travel Planner.

This module provides a web interface for users to generate travel plans
with budget-aware recommendations using an AI agent backend.
"""
...
with st.form(
    key="query_form", clear_on_submit=False
):  # Changed to False to prevent clearing
    ...
    default_index = (
        budget_options.index(st.session_state.current_budget_selection)
    ...
    budget_option = st.radio(
        ...
        budget_options,
        index=default_index,  # Use session state to persist selection
        horizontal=True,
    )
...
if submit_button and user_input.strip():
    ...
    if st.session_state.current_budget_selection is None:
        st.error(
            "⚠️ Please select a budget preference before generating your travel plan."
        )
    else:
        try:
            ...
            budget_preference = budget_map.get(
                st.session_state.current_budget_selection, "budget_friendly"
            )
            ...
            if response.status_code == 200:
                ...
                st.session_state.budget_preference = (
                    st.session_state.current_budget_selection
                )  # Store the full display text
                st.session_state.report_generated = True
                st.session_state.generation_timestamp = (
        ...
        try:
            ...
            export_payload = {
                ...
                "query_info": {
                    ...
                    "budget_preference": st.session_state.get(
                        "budget_preference", "Budget Friendly"
                    ),
                    "generation_timestamp": st.session_state.get(
            ...
            if export_response.status_code == 200:
                ...
                st.success(
                    "✅ Document ready for download! Click the button above to save."
                )
            else:
                st.error("❌ Failed to generate Word document")

        except requests.exceptions.RequestException as e:
            st.error(f"❌ Network error during export: {e}")
        except ValueError as e:
            st.error(f"❌ Invalid export response: {e}")
        except (KeyError, AttributeError, TypeError) as e:
            st.error(f"❌ Export data processing error: {e}")

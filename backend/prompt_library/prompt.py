"""Prompt library for travel planning agents.

This module provides system prompts with budget-aware context for travel planning
agents, allowing customization based on user budget preferences.
"""
...
def get_budget_aware_system_prompt(budget_preference: str = "budget_friendly"):
    ...
    budget_instruction = budget_context.get(
        budget_preference, budget_context["budget_friendly"]
    )
    ...
    return SystemMessage(
        ...
        Provide complete, comprehensive and a detailed travel plan tailored to the selected budget preference. 
        Always try to provide two plans, one for the generic tourist places, another for more 
        ...
        Ensure all recommendations align with the {budget_preference.replace('_', ' ').title()} \
budget preference.
        """
    )
